...
fos_argument_spec = {
    'provider': dict(type='dict', options=fos_provider_spec)
}

def get_provider_argspec():
    return fos_provider_spec

def get_connection(moudle):
...
def get_capabilities(module):
    if hasattr(module, '_fos_capabilities'):
        return module._fos_capabilities
    try:
        capabilities = Connection(module._socket_path).get_capabilities()
    except ConnectionError as exc:
        module.fail_json(msg=to_text(exc, errors='surrogate_then_replace'))
    module._fos_capabilities = json.loads(capabilities)
    return module._fos_capabilities
